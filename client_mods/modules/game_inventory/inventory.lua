...
local function applyInventoryOverlayLayout(forcePosition)
    if not inventoryController or not inventoryController.ui then
        return
    end

    local parent = modules.game_interface.getMainRightPanel()
    if parent and inventoryController.ui:getParent() ~= parent then
        parent:addChild(inventoryController.ui)
    end

    inventoryController.ui:setVisible(true)
    inventoryController.ui:setEnabled(true)
    inventoryController.ui:setWidth(146)
    inventoryController.ui:setDraggable(false)
    inventoryController.ui:setBackgroundColor('#121722cc')
    inventoryController.ui:setBorderColor('#5b8cff44')
    inventoryController.ui:setBorderWidth(1)
...
function inventoryController:onInit()
    refreshInventory_panel()
    inventoryController.ui:setWidth(146)
    inventoryController.ui:setBorderColor('#ffffff22')
    inventoryController.ui:setBorderWidth(1)
    local ui = getInventoryUi()
...
function inventoryController:onGameStart()
    ...
    refreshInventory_panel()
    addEvent(applyInventoryOverlayLayout)
...
function changeInventorySize()
    ...
    refreshInventorySizes()
    addEvent(function() applyInventoryOverlayLayout(true) end)
    modules.game_mainpanel.reloadMainPanelSizes()
    local player = g_game.getLocalPlayer()
...
function extendedView(extendedView)
    ...
    inventoryController.ui.moveOnlyToMain = false
    addEvent(function() applyInventoryOverlayLayout(true) end)
    scheduleEvent(applyInventoryOverlayLayout, 100)
    scheduleEvent(applyInventoryOverlayLayout, 500)
end

function toggle()
    if iconTopMenu and iconTopMenu:isOn() then
