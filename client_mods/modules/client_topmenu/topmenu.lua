...
local compactTopMenu

local allowedRightButtons = {
    logoutButton = true,
    terminalButton = true,
    miniMap = true,
    inventory = true
...
local allowedRightGameButtons = {
    hotkeysWindowButton = true
}

local function shouldKeepButton(panel, id)
    if panel == rightButtonsPanel then
        return allowedRightButtons[id] == true
    end
    if panel == rightGameButtonsPanel then
        return allowedRightGameButtons[id] == true
    end
    return true
end
-- private functions
local function addButton(id, description, icon, callback, panel, toggle, front)
    ...
    if not shouldKeepButton(panel, id) then
        button:hide()
        button:setWidth(0)
        return button
    end
...
function compactTopMenu()
    if not topMenu then return end

    if topLeftButtonsPanel then
        topLeftButtonsPanel:hide()
        topLeftButtonsPanel:setWidth(0)
    end
    if topLeftTogglesPanel then
        topLeftTogglesPanel:hide()
        topLeftTogglesPanel:setWidth(0)
    end
    if leftButtonsPanel then
        leftButtonsPanel:hide()
        leftButtonsPanel:setWidth(0)
    end

    local hideIds = {
        'audioButton', 'optionsButton', 'debugInfoButton', 'hotkeysButton',
        'healthMana', 'highscore', 'manageAccountButton'
    }

    for _, id in ipairs(hideIds) do
        local widget = topMenu:recursiveGetChildById(id)
        if widget then
            widget:hide()
            widget:setWidth(0)
        end
    ...
    local onlinePlayers = topMenu:recursiveGetChildById('topLeftOnlinePlayers')
    if onlinePlayers then onlinePlayers:hide() end
    if topMenu.topLeftDiscord then topMenu.topLeftDiscord:hide() end
    if topMenu.topLeftYoutube then topMenu.topLeftYoutube:hide() end
end
