...
local function applyMinimapOverlayLayout()
    if not mapController or not mapController.ui then
        return
    end

    mapController.ui:setVisible(true)
    mapController.ui:setDraggable(false)
    mapController.ui:setBackgroundColor('#121722cc')
    mapController.ui:setBorderWidth(1)
    mapController.ui:setBorderColor('#5b8cff44')
end
...
function mapController:onGameStart()
    ...
    addEvent(applyMinimapOverlayLayout)
end
...
function extendedView(extendedView)
    ...
    if mainRightPanel and mapController.ui:getParent() ~= mainRightPanel then
