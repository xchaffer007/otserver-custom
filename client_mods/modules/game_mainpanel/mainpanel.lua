...
local ALLOWED_MAINPANEL_BUTTONS = {
    battleButton = true,
    skillsButton = true,
    vipListButton = true,
    spelllistButton = true,
    hotkeysWindowButton = true
}

local ALLOWED_SPECIAL_BUTTONS = {
}
...
function reloadMainPanelSizes()
    ...
    if right_panel.setHeight then
        right_panel:setHeight(total_height)
    end
end
...
local function createButton(id, description, image, callback, special, front, index)
    ...
    if special then
        if not ALLOWED_SPECIAL_BUTTONS[id] then
            button:hide()
            button:setWidth(0)
            button:setHeight(0)
            refreshOptionsSizes()
        ...
        if not ALLOWED_MAINPANEL_BUTTONS[id] then
            button:hide()
            button:setWidth(0)
            button:setHeight(0)
            refreshOptionsSizes()
