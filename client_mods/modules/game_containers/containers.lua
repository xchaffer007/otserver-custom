...
local function applyContainerOverlayLayout(containerWindow, container, forcePosition)
    local root = modules.game_interface.getRootPanel and modules.game_interface.getRootPanel()
    if not root or not containerWindow then
        return false
    end

    if containerWindow:getParent() ~= root then
        containerWindow:setParent(root)
    end

    if forcePosition or not containerWindow._overlayPositionInitialized then
        local offsetY = 0
        if container then
            offsetY = (container:getId() - 1) * 34
        end
        containerWindow:breakAnchors()
        containerWindow:setPosition({
            x = math.max(0, root:getWidth() - containerWindow:getWidth() - 340),
            y = 58 + offsetY
        })
        containerWindow._overlayPositionInitialized = true
    end

    containerWindow:setVisible(true)
    containerWindow:setEnabled(true)
    containerWindow:setFocusable(true)
...
function onContainerOpen(container, previousContainer)
    ...
    if not previousContainer then
        if not applyContainerOverlayLayout(containerWindow, container) then
    ...
    applyContainerOverlayLayout(containerWindow, container)
...
function onContainerChangeSize(container, size)
    ...
    if container.window then
        applyContainerOverlayLayout(container.window, container)
    end
